...
```bash
sudo apt install cmake gcc-mingw-w64-x86-64   # Linux; macOS: brew install mingw-w64
...
make package-windows   # zip it with data/ into build/dist/
```

There's no `install-windows` — copy the `.zip` onto an actual Windows
machine and extract it into `%APPDATA%\obs-studio\plugins\`. See
Packaging and releases below for how `deps-windows.sh` synthesizes
`obs.lib`, and why a native Windows build was ruled out.
...
- **Windows** — zips the staged `bin/64bit/<name>.dll` + `data/` tree
  matching the layout `%APPDATA%\obs-studio\plugins\<name>\` expects on
  an actual Windows machine (there's no `install-windows` target since
...
and publishes the `.dmg`/`.deb`/`.zip` to a GitHub Release whenever a tag
matching `v*` is pushed (the `windows` job runs on the same
`ubuntu-24.04` runner as `linux`, just with `gcc-mingw-w64-x86-64`
installed too) — it does nothing on regular commits/pushes, so it costs
nothing just by existing.
